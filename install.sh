#!/bin/bash
# install.sh
# This script installs zcsldapbackup on a ZCS server. It also makes sure
...
# enforced due to the way the current zcsldapbackup script works.
#
...
# ZcsLdapBackup Defaults - Where the script will be placed and look for its settings
OSE_SRC="/usr/local/bin"
OSE_CONF="/etc/zcsldapbackup"
...
clear
echo "This will install zcsldapbackup, a script aimed at creating backups ldap for ZCS Community Edition."
...
if [ $(id -u) -ne 0 ]; then
	echo "You need root privileges to install zcsldapbackup"
	exit $ERR_NOROOT
fi
...
STATUS=0
MYDIR=`dirname $0`
test -f $MYDIR/src/zcsldapbackup      || STATUS=$ERR_MISSINGFILES
test -f $MYDIR/src/zcsldaprestore      || STATUS=$ERR_MISSINGFILES
test -f $MYDIR/etc/zimbrabackup.conf || STATUS=$ERR_MISSINGFILES
if ! [ $STATUS = 0 ]; then
	printf '[ERROR]\n'
	echo "Some files are missing. Please re-download the ZcsLdapBackup installer."
	exit $STATUS
else
	printf '[OK]\n'
fi
...
# Copy files
install -o $ZIMBRA_USER -m 700 $MYDIR/src/zcsldapbackup $OSE_SRC
install -o $ZIMBRA_USER -m 700 $MYDIR/src/zcsldaprestore $OSE_SRC
